...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function addMessage(messageData) {
        ...
        if (messageData.type === 'bot' && messageData.typing) {
            ...
            if (messageData.type === 'user' && messageData.formData) {
                // 如果是自动演示模式，使用延迟填充表单
                if (isAutoDemo && messageData.delay) {
                    // 使用消息中指定的延迟
                    setTimeout(() => {
                        fillFormData(messageData.formData);
                    }, messageData.delay / 2); // 使用一半的延迟时间填充表单，让用户有时间看到消息
                } else {
                    // 非自动演示模式，使用较短的延迟
                    setTimeout(() => {
                        fillFormData(messageData.formData);
    ...
    function sendUserMessage() {
        ...
        if (message) {
            ...
            // 如果是关于重大疾病保险的消息，使用更长的延迟
            if (message.includes('重大疾病保险') || message.includes('重疾')) {
                responseDelay = 5000; // 使用与演示对话中相同的延迟
                
                // 尝试查找匹配的预设消息，获取其表单数据和延迟
                for (const demoMsg of demoConversation) {
                    if (demoMsg.type === 'user' && demoMsg.message.includes('重大疾病保险')) {
                        // 如果找到匹配的预设消息，使用其延迟和表单数据
                        if (demoMsg.delay) {
                            responseDelay = demoMsg.delay;
                        }
                        
                        // 如果有表单数据，应用它
                        if (demoMsg.formData) {
                            setTimeout(() => {
                                fillFormData(demoMsg.formData);
                            }, 200);
                        }
        ...
        if (currentMessageIndex < demoConversation.length) {
            ...
            autoDemoTimer = setTimeout(() => {
                if (nextMessage.type === 'user') {
                    ...
                    // 延迟后显示机器人回复
                    // 使用用户消息中的delay属性，如果存在的话
                    const userMessageDelay = nextMessage.delay || 1500;
